...
namespace HospitalDAL
{
    public class DataAccess
    {
        ...
        public History<Patient> patientHistory = new History<Patient>("Patient");
        public History<Doctor> doctorHistory = new History<Doctor>("Doctor");
        public History<Appointment> appointmentHistory = new History<Appointment>("Appointment");
        ...
        private bool DoesDoctorExist(int doctorId)
        {
            string connString = @"Data Source=(localdb)\ProjectModels;Initial Catalog=Hospital;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False";
        ...
        {
            DeletionHistoryManager d = new DeletionHistoryManager();
            d.SavePatientData(patientId);
            string connString = @"Data Source=(localdb)\ProjectModels;Initial Catalog=Hospital;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False";

            DeletionHistoryManager deletionHistoryManager = new DeletionHistoryManager();

            Patient patientToDelete = GetPatientById(patientId); 
            if (patientToDelete != null)
            {
                deletionHistoryManager.SavePatientDeletionRecord(patientToDelete);
            ...
            {
                Console.WriteLine($"No patient found with ID {patientId}. Deletion operation aborted.");
            }
        }
        public void DeleteDoctorFromDatabase(int doctorId)
        {
            DeletionHistoryManager d = new DeletionHistoryManager();
            d.SaveDoctorData(doctorId);
            string connString = @"Data Source=(localdb)\ProjectModels;Initial Catalog=Hospital;Integrated Security=True;Connect Timeout=30;Encrypt=False;Trust Server Certificate=False;Application Intent=ReadWrite;Multi Subnet Failover=False";

            DeletionHistoryManager deletionHistoryManager = new DeletionHistoryManager();

            Doctor doctorToDelete = d.GetDoctorById(doctorId);

            if (doctorToDelete != null)
            {
                d.SaveDoctorDeletionRecord(doctorToDelete);
            ...
            {
                Console.WriteLine($"No doctor found with ID {doctorId}. Deletion operation aborted.");
            }
        ...
        public void DeleteAppointmentFromDatabase(int appointmentId)
        {
            ...
            {
                deletionHistoryManager.SaveAppointmentDeletionRecord(appointmentToDelete);
